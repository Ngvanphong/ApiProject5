...
// Type: ApiProject5.DuctPipe.DuctPipeBinding
...
namespace ApiProject5.DuctPipe
{
  ...
  public class DuctPipeBinding : IExternalCommand
  {
      ...
      ElementSet elements)
    {
      ...
      try
      {
        num1 = double.Parse(StatisAngleViewer.StatisViewerShow);
      }
      catch
      {
      }
      ...
      {
        int num2 = (int) TaskDialog.Show("Error", "You must choose a duct or pipe");
      ...
      if (num1 != 90.0)
      {
        double num2 = Math.Abs(endPoint1.Z - endPoint3.Z) / Math.Tan(num1 * Math.PI / 180.0);
      ...
      if (category.Name == "Ducts")
      {
        ...
        try
        {
          ...
          else
          {
            try
            {
              Duct newDuctCus2 = this.CreateNewDuctCus(document, xyz5, xyz3, ductSystem, levelId, ductType);
              this.ChangeSizeDuct(document, diameter1, width, height, newDuctCus2);
              if (newDuctCus2 != null && !xyz1.IsAlmostEqualTo(source) && !xyz1.IsAlmostEqualTo(-source))
              {
                this.CreateConnectEbow(document, newDuctCus2, xyz5, newDuctCus1, xyz5);
                using (TransactionGroup transactionGroup = new TransactionGroup(document, "groupConnectDuctTee"))
                {
                  ...
                  try
                  {
                    this.CreateConectTree(document, newDuctCus2, xyz3, duct2_1);
      ...
      else
      {
        ...
        Pipe newPipeCus1 = this.CreateNewPipeCus(document, xyz2, xyz5, pipeSystem, levelId, pipeType);
        this.ChangeSizePipe(document, diameter2, width, height, newPipeCus1);
        Pipe pipe2_2 = pipe;
        if (newPipeCus1 != null)
          this.CreateConnectEbowPipe(document, newPipeCus1, xyz2, pipe2_2, xyz2);
        try
        {
          if (xyz1.IsAlmostEqualTo(source) || xyz1.IsAlmostEqualTo(-source))
          {
            this.CreateConnectEbowPipe(document, newPipeCus1, xyz5, pipe2_1, xyz3);
          }
          else
          {
            try
            {
              ...
              if (newPipeCus2 != null && !xyz1.IsAlmostEqualTo(source) && !xyz1.IsAlmostEqualTo(-source))
              {
                ...
                using (TransactionGroup transactionGroup = new TransactionGroup(document, "groupConnectPipeTee"))
                {
                  ...
                  try
                  {
                    this.CreateConectTreePipe(document, newPipeCus2, xyz3, pipe2_1);
                    int num7 = (int) transactionGroup.Commit();
                  }
                  catch
                  {
    ...
    {
      Duct duct = (Duct) null;
      using (Transaction transaction = new Transaction(doc, "CreateDuctPipe2"))
      {
    ...
    {
      Pipe pipe = (Pipe) null;
      using (Transaction transaction = new Transaction(doc, "CreateDuctPipe3"))
      {
        int num1 = (int) transaction.Start();
    ...
    public void CreateConectTree(Document doc, Duct duct1, XYZ G, Duct duct2)
    {
      ...
      using (Transaction transaction = new Transaction(doc, "TreeDuctConnect"))
      {
        int num2 = (int) transaction.Start();
    ...
    public void CreateConectTreePipe(Document doc, Pipe pipe1, XYZ G, Pipe pipe2)
    {
      ...
      XYZ xyz = endPoint2.Subtract(endPoint1).Normalize();
      XYZ ptBreak = endPoint1.Add(xyz.Multiply(num1));
      using (Transaction transaction = new Transaction(doc, "DeviePipe"))
      {
        int num2 = (int) transaction.Start();
      ...
      using (Transaction transaction = new Transaction(doc, "TreePipeConnect"))
      {
        int num2 = (int) transaction.Start();
        doc.Create.NewTeeFitting(connector1, connector2, connector3);
        int num3 = (int) transaction.Commit();
    ...
    public void ChangeSizeDuct(
      ...
      Duct element)
    {
      using (Transaction transaction = new Transaction(doc, "SetSizeDuct"))
      {
        int num1 = (int) transaction.Start();
        if (diameter != 0.0)
        {
